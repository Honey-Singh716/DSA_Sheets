...
int count(vector<int>& weight,vector<int>& val,int idx,int limit,vector<vector<int>>& dp){
    ...
    if(weight[idx] <= limit){
        take = val[idx] + count(weight,val,idx,limit-weight[idx],dp);
    }
...
int main(){
    ...
    //1. base case
    // for(int i= weight[0];i<=limit;i++)  dp[0][i] = (i/weight[0])*val[0]; // prev[i] = val[0];
    ...
    //         int take = INT_MIN;
    //         if(weight[idx] <= w){
    //             take = val[idx] + dp[idx][w - weight[idx]]; // prev[w-weight[idx]];
    //         }
